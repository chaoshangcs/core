...
#        print "masked distance matrix:", sess.run(masked_distance_matrix)
#        print "tiled image vals:", (sess.run(tiled_image_vals))
        ...
        print "tiled image copy", sess.run(tiled_image_copy)
        ...
        print "copy atm labels:", sess.run(copy_atm_labels)

        tile_image = tf.tile(tf.cast(x_image_coords, tf.int32), [number_of_atoms, 1])
        print "tile image 1:", sess.run(tile_image)
        ...
        print "shape of the new dense tensor:", sess.run(tf.shape(new_dense_3d))
        result = tf.nn.conv3d(tf.cast(new_dense_3d, tf.float32), self.kernel, self.strides, "VALID")
        print "shape of the result:", sess.run(tf.shape(result))
        return result

#Test
testOneInput = tf.SparseTensor([[1, 2, 1], [2, 3, 2], [10, 11, 10], [11, 12, 11]], [1, 3, 5, 10], [20, 20, 20])
