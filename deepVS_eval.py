...
from deepVS_input import read_receptor_and_ligand,index_the_database_into_queue
...
def evaluate_on_train_set():
    ...
    y_conv = deepVS_net(ligand_atoms, ligand_coords, receptor_elements, receptor_coords, keep_prob)

    # compute softmax over (single prediction?) raw predictions
    predictions = tf.nn.softmax(y_conv)[:1]

    # Expand dimensions to use as a batch of one ex
    ligand_file = tf.expand_dims(ligand_file, axis=0)
    current_epoch = tf.expand_dims(current_epoch, axis=0)
    ...
    try:
        while True or not coord.should_stop():
            ...
            my_ligand_files, my_ligand_frames, my_predictions, my_labels = sess.run(
                [ligand_file, current_epoch, predictions, label],
